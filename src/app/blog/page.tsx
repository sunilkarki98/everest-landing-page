...
// Dummy static data for Phase 1
const blogCategories = ["All", "Visa Updates", "Scholarships", "Life Abroad", "University Admissions"];
        ...
        <Container>
                ...
                <div className="bg-white rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col lg:flex-row transition-all duration-300 hover:shadow-2xl">
                  <div className="w-full lg:w-3/5 h-[300px] lg:h-[450px] relative overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 flex gap-3">
                      <Badge className="bg-accent text-primary border-none text-sm font-bold shadow-lg">Featured</Badge>
                      <Badge className="bg-white/90 text-primary border-none shadow-lg">{featuredArticle.category}</Badge>
                    </div>
                  </div>
                  <div className="w-full lg:w-2/5 p-8 lg:p-12 flex flex-col justify-center">
                    <div className="flex items-center gap-4 text-slate-500 text-sm font-medium mb-4">
                      ...
                      <div className="flex items-center gap-1.5"><Clock size={16} /> {featuredArticle.readTime}</div>
                    </div>
                    ...
                    <div className="flex items-center text-accent font-bold mt-auto group-hover:translate-x-2 transition-transform">
                      Read Full Article <ArrowRight size={20} className="ml-2" />
                    </div>
                  </div>
                </div>
              </Link>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArticles.map((article, index) => (
              ...
              >
                <Link href={`/blog/${article.slug}`} className="group flex flex-col h-full bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl border border-slate-100 transition-all duration-300">
                  ...
                  <div className="p-6 md:p-8 flex flex-col flex-grow">
                    ...
                    <div className="mt-auto pt-6 border-t border-slate-100 flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{article.author}</span>
          ...
          {filteredArticles.length === 0 && (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-100">
              ...
              <p className="text-slate-500">Check back later for more updates in this category.</p>
            </div>
          )}
        </Container>
      </section>
